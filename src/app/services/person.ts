...
export const getPersonByUserId = async (id: UUID) => {
    const response = await apiRequest<PersonResponse>(`/persons/user/${id}`, {method: "GET"});
    if (!response.success) {
...
export const getPersonById = async (id: number) => {
    const response = await apiRequest<PersonResponse>(`/persons/${id}`, {method: "GET"});
    if (!response.success) {
        throw new Error(`Failed to fetch person. ${response.message}`);
    }
    return response.data;
...
export const getAllPersons = async () => {
    const response = await apiRequest<PersonsResponse>(`/persons`, { method: "GET"});
    if(!response.success) {
