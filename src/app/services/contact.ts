...
export const getContactById = async (id: number ) =>  {
 const response = await apiRequest<ContactResponse>(`/contacts/${id}`, { method: "GET"});
    if(!response.success) {
...
export const getAllContactsByPersonId = async (id: number ) =>  {
 const response = await apiRequest<ContactsResponse>(`/contacts/person/${id}`, { method: "GET"});
    if(!response.success) {
        throw new Error(`Failed to fetch contacts. ${response.message}`)
    }
    return response.data;
...
export const getAllContactsByUserId = async (id: UUID ) =>  {
 const response = await apiRequest<ContactsResponse>(`/contacts/user/${id}`, { method: "GET"});
    if(!response.success) {
        throw new Error(`Failed to fetch contacts. ${response.message}`)
    }
    return response.data;
...
export const getAllContacts = async () => {
    const response = await apiRequest<ContactsResponse>(`/contacts`, { method: "GET"});
    if(!response.success) {
