...
export default function CalculatorModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="modal fixed inset-0 bg-black/60 flex items-center justify-center">
      <div
        ref={modalRef}
        className="modal-card calculator absolute w-[92%] max-w-[520px] glass"
      >
        <div
          className="calc-header bg-white/10 p-1.5 rounded-t-lg text-center font-semibold"
        ...
        <div className="calc-grid grid grid-cols-4 gap-2">
          {[
            ...
            <button
              ...
              className="p-2.5 border border-white/10 bg-white/6 rounded-10"
            >
              {key}
            </button>
          ))}
