          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await onCreateUser(userToSave);
              } finally {
                setIsLoading(false);
              }
            }}
            disabled={isLoading}
            className={theme?.primaryColor ? "" : "bg-accent-600 hover:bg-accent-700"}
            style={theme?.primaryColor ? {
              backgroundColor: isLoading ? theme.primaryColor : theme.primaryColor,
              opacity: isLoading ? 0.7 : 1,
            } : undefined}
            onMouseEnter={(e) => {
              if (theme?.primaryHoverColor && !isLoading) {
                e.currentTarget.style.backgroundColor = theme.primaryHoverColor;
              }
            }}
            onMouseLeave={(e) => {
              if (theme?.primaryColor && !isLoading) {
